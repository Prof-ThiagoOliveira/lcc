...
##' @title Internal function to prepare \code{\link[lcc]{lccModel}} function
##'
##' @description This is an internally called function used to verify the specification of variance-covariance matrices and likelihood based method.
##'
##' @usage NULL
##'
##' @importFrom nlme lme
...
init<-function(var.class, weights.form, REML, qf, qr, pdmat, dataset, resp, subject, method, time){
  ...
  if(is.null(var.class)==FALSE){
    ...
    if(is.null(var.class)==FALSE){
      ...
      if(is.null(weights.form)) stop("Please specify the 'weights.form' argument.", call.=FALSE)
      if(weights.form=="time.ident" || weights.form == "method") {
        vc<-class(var.class())[1]
        if(vc != "varIdent") stop("Please specify the 'weight.form' correctly for varExp class", call.=FALSE)
    }

    if(weights.form=="time" || weights.form == "both") {
      vc<-class(var.class())[1]
      if(vc != "varExp") stop("Please specify the 'weight.form' correctly for varIdent class", call.=FALSE)
    }

    if(weights.form != "time" && weights.form != "method" &&
       weights.form != "time.ident" && weights.form != "both") stop("The weights.form argument are \"time\", \"method\", \"time.ident\", or \"both\".", call. = FALSE)
     if(vc != "varIdent" && vc != "varExp") {
      stop("Method only implemented for classes varIdent and varExp", call.=FALSE)
      }
    }
  if(REML==TRUE){
    MethodREML<-"REML"
  ...
  if(qf<=0) stop("polynomial degree should be greater or equal to 1 in the fixed part.", call.=FALSE)
  if(qr<0)  stop("polynomial degree should be greater or equal to 0 in the random part.", call.=FALSE)
  ...
  initList<-list("MethodREML" = MethodREML, "pdmat" = pdmat, "var.class" = var.class)
