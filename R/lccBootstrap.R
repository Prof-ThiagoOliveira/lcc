...
#                                                                                         #
# File: lccBootstrap.R                                                                    #
# Contains: dataBootstrap, bootstrapSamples, lccBootstrap, lpcBootstrap, laBootstrap      #
#                                                                                         #
...
#                                                                                         #
###########################################################################################
##' @title Internal functions to generate bootstrap samples based on dataset.
##'
##' @description This is an internally called functions used to generate bootstrap samples.
##'
##' @usage NULL
##'
...
##' @title Internal functions to estimate fixed effects and variance components.
##'
##' @description This is an internally called functions used to estimate fixed effects and variance components for each bootstrap sample.
##'
##' @usage NULL
##'
##' @author Thiago de Paula Oliveira, \email{thiago.paula.oliveira@@usp.br}
...
bootstrapSamples<-function(nboot, model, q_f, q_r, interaction, covar, var.class, pdmat, weights.form, show.warnings, lme.control, method.init){
  ...
  cat("Convergence error in", warnings, "out of", nboot, "bootstrap samples.", "\n")
  return(lcc.bootstrap)
}

##' @title Internal functions to generate longitudinal concordance correlation samples.
##'
##' @description This is an internally called functions used to generate longitudinal concordance correlation samples.
##'
##' @usage NULL
##'
##' @author Thiago de Paula Oliveira, \email{thiago.paula.oliveira@@usp.br}
...
lccBootstrap<-function(model_boot, diff_boot, ldb, nboot, tk, q_f){
  ...
  if(ldb == 1) {
    for(i in 1:nboot){
      CCC_Boot[[i]] <- lccWrapper(model = model_boot[[i]], q_f = q_f, n.delta = 1,
    ...
    if(nd<=1){
      ...
      for(i in 1:nboot){
        for(j in 1:ldb)  {
          CCC_l[[j]] <- lccWrapper(model = model_boot[[i]], q_f = q_f, n.delta = 1,
                                    tk = tk, diffbeta = as.numeric(diff_boot[[i]][[j]]))
        }
        CCC_Boot[[i]]<-CCC_l
      }
    } else{
      ...
      for(i in 1:nboot){
        for(j in 1:ldb)  {
          CCC_l[[j]] <- lccWrapper(model = model_boot[[i]], q_f = q_f, n.delta = j,
                                    tk = tk, diffbeta = as.numeric(diff_boot[[i]][[j]]))
        }
        CCC_Boot[[i]]<-CCC_l
      }
...
##' @title Internal functions to generate longitudinal Pearson correlation samples.
##'
##' @description This is an internally called functions used to generate longitudinal Pearson correlation samples.
##'
##' @usage NULL
##'
##' @author Thiago de Paula Oliveira, \email{thiago.paula.oliveira@@usp.br}
...
lpcBootstrap<-function(model_boot, ldb, nboot, tk, q_f){
  ...
  if(ldb == 1) {
    for(i in 1:nboot){
      LPC_Boot[[i]] <- lpcWrapper(model = model_boot[[i]], q_f = q_f, n.delta = 1,
                                   tk = tk)
    }
    return(LPC_Boot)
  } else {
    ...
    if(nd<=1){
      ...
      for(i in 1:nboot){
        for(j in 1:ldb)  {
          LPC_l[[j]] <- lpcWrapper(model = model_boot[[i]], q_f = q_f, n.delta = 1,
                                    tk = tk)
        }
        LPC_Boot[[i]]<-LPC_l
      }
      ...
      for(i in 1:nboot){
        for(j in 1:ldb)  {
          LPC_l[[j]] <- lpcWrapper(model = model_boot[[i]], q_f = q_f, n.delta = j,
                                    tk = tk)
        }
        LPC_Boot[[i]]<-LPC_l
      }
...
##'
##' @description This is an internally called functions used to generate longitudinal caccuracy samples.
##'
##' @usage NULL
##'
##' @author Thiago de Paula Oliveira, \email{thiago.paula.oliveira@@usp.br}
...
laBootstrap<-function(model_boot, diff_boot, ldb, nboot, tk, q_f){
  ...
  if(ldb == 1) {
    for(i in 1:nboot){
      Cb_Boot[[i]] <- laWrapper(model = model_boot[[i]], q_f = q_f, n.delta = 1,
                                 tk = tk, diffbeta = as.numeric(diff_boot[[i]][[1]]))
    }
    return(Cb_Boot)
  } else {
    ...
    if(nd<=1){
      ...
      for(i in 1:nboot){
        for(j in 1:ldb)  {
          Cb_l[[j]] <- laWrapper(model = model_boot[[i]], q_f = q_f, n.delta = 1,
                                  tk = tk, diffbeta = as.numeric(diff_boot[[i]][[j]]))
        }
        Cb_Boot[[i]]<-Cb_l
      }
      ...
      for(i in 1:nboot){
        for(j in 1:ldb)  {
          Cb_l[[j]] <- laWrapper(model = model_boot[[i]], q_f = q_f, n.delta = j,
                                  tk = tk, diffbeta = as.numeric(diff_boot[[i]][[j]]))
        }
        Cb_Boot[[i]]<-Cb_l
      }
